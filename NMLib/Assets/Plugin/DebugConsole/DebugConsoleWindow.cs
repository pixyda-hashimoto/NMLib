...
public class DebugConsoleWindow : MonoBehaviour
{
	...
	// デバッグコマンド
	private Dictionary<string,ConsoleCommandBase> _debugCommands = new Dictionary<string, ConsoleCommandBase>();
	...
	private void ExecInputCommand( string command )
	{
		...
		if( _debugCommands.ContainsKey( split[0] ) == true ) {
			int ret = _debugCommands[split[0]].ExecCommand( split );
			if( ret < 0 ) {
